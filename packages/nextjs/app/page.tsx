...
import { useAccount, useBalance, useReadContract, useWriteContract, usePublicClient, useWatchContractEvent } from "wagmi";
...
const Home: NextPage = () => {
  ...
  const { data: projects, isLoading: isLoadingProjects, error: projectsError } = useScaffoldReadContract({
        ...
        {!activeRole ? (
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 max-w-5xl mx-auto mt-4">
            {[
              ...
              <div key={btn.role} onClick={() => setActiveRole(btn.role as any)} className={`group cursor-pointer bg-white p-10 rounded-[3rem] border-2 border-slate-100 ${btn.color} transition-all text-center shadow-sm hover:shadow-xl transform hover:-translate-y-2`}>
          ...
          <div className="space-y-8 pb-12">
            <div className="flex items-center gap-4">
              <button onClick={() => setActiveRole(null)} className="px-4 py-2 bg-white border border-slate-200 rounded-xl font-black text-[9px] uppercase tracking-widest text-slate-400 hover:text-blue-600 transition-all shadow-sm">
                ← Menú
              </button>
              <div className="relative flex-grow max-w-xs">
                <input id="searchTerm" name="searchTerm" type="text" placeholder="Buscar..." className="w-full px-10 py-2 bg-white border border-slate-200 rounded-xl font-bold text-xs outline-none focus:border-blue-500 transition-all shadow-sm" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} />
              ...
              <section className="bg-white p-6 rounded-[2rem] border border-slate-100 shadow-sm">
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
                  <div className="space-y-1">
                    ...
                    <input id="projectName" name="projectName" className="w-full h-10 px-4 bg-slate-50 border border-slate-100 rounded-xl font-bold text-xs" placeholder="Nombre..." value={projectName} onChange={e => setProjectName(e.target.value)} />
                  </div>
                  <div className="space-y-1">
                    <label className="text-[9px] font-black uppercase text-slate-400 ml-1 italic">Meta (ETH)</label>
                    <input id="projectTarget" name="projectTarget" className="w-full h-10 px-4 bg-slate-50 border border-slate-100 rounded-xl font-bold text-xs" type="number" step="0.0001" min="0.0001" placeholder="Ej: 0.1 (Medalla Oro)" value={projectTarget} onChange={e => setProjectTarget(e.target.value)} />
                  </div>
                  <div className="space-y-1">
                    <label className="text-[9px] font-black uppercase text-slate-400 ml-1 italic">Días</label>
                    <input id="projectDays" name="projectDays" className="w-full h-10 px-4 bg-slate-50 border border-slate-100 rounded-xl font-bold text-xs" type="number" value={projectDays} onChange={e => setProjectDays(e.target.value)} />
                  </div>
                  <button className={`h-10 text-white rounded-xl font-black uppercase text-[9px] shadow-md transition-all ${(isCreating || !isConnected || !projectName || Number(projectDays) <= 0) ? "bg-slate-300" : "bg-blue-600 hover:bg-blue-700"}`}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
              {isLoadingProjects ? (
                <div className="col-span-full text-center py-10 font-black text-slate-300 uppercase italic">Escaneando...</div>
              ) : projectsError ? (
                <div className="col-span-full text-center py-10 font-black text-red-500 uppercase italic">Error de conexión con el contrato</div>
              ) : filteredProjects?.length === 0 ? (
                <div className="col-span-full text-center py-10 font-black text-slate-400 uppercase italic">No hay proyectos activos</div>
              ) : filteredProjects?.map((project, index) => (
                <ProjectCard key={project.contractAddress || index} project={project} role={activeRole} connectedAddress={connectedAddress} interactWithProject={interactWithProject} states={{ donationAmount, setDonationAmount, withdrawReason, setWithdrawReason }} mounted={mounted} />
              ))}
            </div>

            <section className="bg-white p-6 rounded-[2rem] shadow-sm border border-slate-100 mt-10">
              <h2 className="text-lg font-black mb-4 uppercase italic tracking-tighter">📊 Auditoría General del Sistema</h2>
              <div className="overflow-x-auto">
                <table className="w-full border-separate border-spacing-y-2">
                  <thead>
                    <tr>
                      <th className="px-4 py-2 text-[9px] font-black text-slate-400 text-left uppercase italic">Proyecto</th>
                      <th className="px-4 py-2 text-[9px] font-black text-slate-400 text-left uppercase italic">Acción / Razón</th>
                      <th className="px-4 py-2 text-[9px] font-black text-slate-400 text-right uppercase italic">Monto</th>
                    </tr>
                  </thead>
                  <tbody>
                    {filteredProjects?.map((p, idx) => (
                      <GlobalAuditRow key={p.contractAddress || idx} project={p} />
                    ))}
                  </tbody>
                </table>
              </div>
...
const GlobalAuditRow = ({ project }: { project: Projectoview }) => {
  const validAddr = project?.contractAddress && project.contractAddress.startsWith("0x") ? project.contractAddress as `0x${string}` : undefined;
  ...
  useEffect(() => {
    ...
    const fetchLogs = async () => {
      try {
        const contributionAbi = crowdfundingProAbi.find((item: any) => item.type === "event" && item.name === "ContributionReceived");
        const refundAbi = crowdfundingProAbi.find((item: any) => item.type === "event" && item.name === "RefundClaimed");
        const fromBlock = 0n;
        ...
        const [contributionLogs, refundLogs] = await Promise.all([
          ...
          refundAbi ? publicClient.getLogs({
            address: validAddr,
            event: refundAbi,
            fromBlock: fromBlock,
          }) : Promise.resolve([])
        ...
        const allLogs = [...contributionLogs, ...refundLogs].sort((a: any, b: any) =>
          (Number(b.blockNumber) - Number(a.blockNumber)) || (Number(b.logIndex) - Number(a.logIndex))
        );
  ...
  return (
    <>
      {Array.isArray(withdrawals) && withdrawals.map((w: any, i: number) => (
        <tr key={`w-${project.contractAddress}-${i}`} className="bg-amber-50/50 rounded-xl text-[11px]">
          <td className="px-4 py-3 font-bold uppercase italic border-l-4 border-amber-500 text-slate-500">{project.name}</td>
      ...
      {Array.isArray(events) && events.map((event: any, i: number) => {
        const isRefund = event.eventName === "RefundClaimed";
        return (
          <tr key={`${event.transactionHash}-${i}`} className={`${isRefund ? "bg-red-50/50" : "bg-slate-50"} rounded-xl text-[11px]`}>
            ...
            <td className={`px-4 py-2 text-right font-black ${isRefund ? "text-red-600" : "text-blue-600"}`}>
              {isRefund ? "-" : "+"}{Number(Number(formatEther(event.args?.amount || 0n)).toFixed(4))} ETH
            </td>
          </tr>
        );
      })}
...
const ProjectCard = ({ project, role, connectedAddress, interactWithProject, states, mounted }: any) => {
  ...
  const crowdfundingProAbi = targetNetwork.id ? (deployedContracts as any)[targetNetwork.id as keyof typeof deployedContracts]?.CrowdfundingPro?.abi : [];
    ...
    <div className="bg-white rounded-[2rem] border border-slate-100 flex flex-col min-h-[550px] transition-all hover:shadow-lg overflow-hidden">
      <div className="p-6 flex-grow">
        <div className="flex justify-between items-center mb-4">
          <span className={`px-3 py-1 rounded-lg text-[8px] font-black uppercase border ${isExpired && !isSuccess ? 'bg-red-600 text-white border-red-700' : isSuccess ? 'bg-amber-50 text-amber-600 border-amber-100' : 'bg-green-50 text-green-600 border-green-100'}`}>
        ...
        <h3 className="text-xl font-black text-[#1E293B] mb-4 uppercase italic leading-tight truncate">{project.name}</h3>
        ...
        <div className="mb-6 p-3 bg-slate-50 rounded-2xl flex justify-around items-center border border-slate-100 shadow-inner min-h-[90px]">
          {[{ t: 0.001, e: "🥉" }, { t: 0.01, e: "🥈" }, { t: 0.1, e: "🥇" }].map(m => (
            <div key={m.t} className={`flex flex-col items-center transition-all duration-500 ${raised >= m.t ? 'opacity-100 scale-125 animate-bounce' : 'opacity-20 grayscale'}`}>
              <span className="text-2xl mb-1">{m.e}</span>
              <span className={`text-[7px] font-black uppercase ${raised >= m.t ? 'text-blue-600' : 'text-slate-400'}`}>{m.t} ETH</span>
            </div>
          ))}
        </div>
        ...
        <div className="space-y-2">
          <div className="flex justify-between items-end">
            <div className="flex flex-col text-xl font-black">
              ...
              <span>
                {Number(raised.toFixed(4))}<span className="text-slate-300 mx-1">/</span><span className="text-blue-500">{Number(targetEth.toFixed(4))}</span>
              </span>
            </div>
            <span className="text-sm font-black text-blue-600 font-mono">{percent.toFixed(0)}%</span>
          </div>
          <div className="w-full h-2 bg-slate-100 rounded-full overflow-hidden">
            <div className={`h-full rounded-full transition-all duration-1000 ${isSuccess ? 'bg-green-500' : isExpired && !isSuccess ? 'bg-red-500' : 'bg-blue-600'}`} style={{ width: `${percent}%` }} />
          </div>
        </div>

        <div className="mt-6 pt-4 border-t border-slate-50">
          <h4 className="text-[8px] font-black uppercase text-slate-400 mb-2 italic text-center">Auditoría de Fondos</h4>
          <div className="max-h-32 overflow-y-auto space-y-2">
            {Array.isArray(withdrawals) && withdrawals.length > 0 ? (withdrawals as any[]).map((w, idx) => (
              <div key={idx} className="bg-amber-50/60 p-2 rounded-xl border border-amber-100 text-center">
                <p className="font-black italic text-[11px] text-slate-800 uppercase leading-none">"{w.reason}"</p>
                <p className="font-black text-amber-600 text-[13px] mt-1">{formatEther(w.amount || 0n)} ETH</p>
              </div>
            )) : <p className="text-[8px] text-center text-slate-300 italic font-black uppercase">Sin retiros registrados</p>}
          </div>
        </div>
      </div>
          ...
          {role === "admin" && isOwner ? (
            <div className="space-y-2">
              {isSuccess && currentBalance > 0 ? (
                <>
                  <input id={`withdrawReason-${addr}`} name={`withdrawReason-${addr}`} placeholder="Motivo del retiro..." className="w-full h-8 bg-white border border-slate-200 rounded-lg px-3 text-[10px] font-bold" value={states.withdrawReason[addr] || ""} onChange={e => states.setWithdrawReason({ ...states.withdrawReason, [addr]: e.target.value })} />
                  <button className="w-full h-8 bg-slate-900 text-white rounded-lg font-black text-[9px] uppercase shadow-sm"
                    disabled={loading || !states.withdrawReason[addr]}
                    onClick={async () => {
                      setLoading(true);
                      try {
                        await interactWithProject({ functionName: "withdrawFunds", address: addr, args: [states.withdrawReason[addr], parseEther(currentBalance.toString())] });
                        states.setWithdrawReason({ ...states.withdrawReason, [addr]: "" });
                        notification.success("Retiro auditado con éxito");
                      } catch (e) { console.error(e); } finally { setLoading(false); }
                    }}>Retirar y Auditar</button>
                </>
              ) : (
            ...
            <div className="space-y-2">
              {!isExpired && !isSuccess ? (
                <div className="flex gap-2">
                  <input id={`donationAmount-${addr}`} name={`donationAmount-${addr}`} type="number" step="0.001" className="w-[40%] h-10 bg-white border border-slate-200 rounded-xl px-3 font-black text-xs" placeholder="Ej: 0.1 Oro" value={states.donationAmount[addr] || ""} onChange={e => states.setDonationAmount({ ...states.donationAmount, [addr]: e.target.value })} />
                  <button className="w-[60%] h-10 bg-green-600 text-white rounded-xl font-black text-[9px] uppercase shadow-sm"
                    ...
                    onClick={async () => {
                      ...
                      setLoading(true);
                      try {
                        await interactWithProject({ functionName: "contribute", address: addr, value: parseEther(states.donationAmount[addr]) });
                ...
                <button className="w-full h-10 bg-red-600 text-white rounded-xl font-black text-[9px] uppercase shadow-md hover:bg-red-700"
                  onClick={async () => {
                    setLoading(true);
                    try {
